...
class PrivacyRequestService {
  ...
  async transition({ requestId, nextStatus, failureCode = null, assignedAdminId = null }) {
    ...
    return PrivacyRequest.sequelize.transaction(async (transaction) => {
      ...
      if (nextStatus === 'READY' || nextStatus === 'COMPLETED') values.completedAt = now;
  ...
  async processAccess({userId,requestId}) { 
    const m=getModels(); 
    return m.PrivacyRequest.sequelize.transaction(async(transaction)=>{
      const r=await m.PrivacyRequest.findOne({where:{id:requestId,userId,requestType:'ACCESS'},transaction,lock:transaction.LOCK.UPDATE});
      if(!r){const e=new Error('Privacy request not found.');e.status=404;e.code='NOT_FOUND';throw e;}
      if(r.status!=='VERIFIED'){const e=new Error('Privacy request is not ready.');e.status=409;e.code='PRIVACY_REQUEST_NOT_READY';throw e;}
      r.status='PROCESSING';
      r.processingStartedAt=new Date();
      await r.save({transaction});
      
      try {
        const [u,p,d,n,c,history]=await Promise.all([
          m.User.findByPk(userId,{attributes:['id','name','email','phoneNumber','authProvider','isVerified','accountStatus','createdAt'],transaction}),
          m.OnboardingProfile.findOne({where:{userId},attributes:['birthDate','gender','city','profession','education','hometown','interests','languages','createdAt','updatedAt'],transaction}),
          m.DiscoverFilterPreference.findOne({where:{userId},attributes:['minAge','maxAge','maxDistanceKm','minScore','city','verifiedOnly','onlineNow'],transaction}),
          m.NotificationPreference.findOne({where:{userId},attributes:['newMatches','messages','eventReminders','offers','pushEnabled','emailEnabled','smsEnabled'],transaction}),
          m.ConsentEvent.findAll({where:{userId},attributes:['documentVersionId','purpose','action','source','platform','occurredAt'],order:[['occurredAt','ASC']],transaction}),
          m.PrivacyRequest.findAll({where:{userId},attributes:['id','requestType','status','requestedAt','identityVerifiedAt','processingStartedAt','completedAt','failedAt','failureCode','correlationId'],order:[['requestedAt','ASC']],transaction})
        ]);
        ...
        if (m.Match) {
          try {
            ...
            const matches = await m.Match.findAll({ where: { [Op.or]: [{ user1Id: userId }, { user2Id: userId }] }, attributes: ['id', 'status', 'createdAt', 'updatedAt'], transaction });
            chatMetadata = matches.map(m => m.toJSON());
          } catch (e) { /* ignore if model not ready */ }
        }

        const data={
          account:u?.toJSON()||null,
          onboardingProfile:p?.toJSON()||null,
          discoveryPreferences:d?.toJSON()||null,
          notificationPreferences:n?.toJSON()||null,
          consentHistory:c.map(x=>x.toJSON()),
          privacyRequestHistory:history.map(safeRequest),
          chatMetadata
        };
        ...
        r.status='FAILED';
        r.failedAt=new Date();
        r.failureCode='ACCESS_PROCESSING_FAILED';
        await r.save({transaction});
        throw error;
      }
    }); 
