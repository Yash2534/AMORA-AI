...
module.exports = (err, _req, res, _next) => {
  ...
  if (Number.isInteger(err.status) && err.status >= 400 && err.status < 600) return res.status(err.status).json({ success: false, message: err.message, code: err.code || 'REQUEST_FAILED', errors: [] });
