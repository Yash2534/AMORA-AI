...
const { getModels } = require('../models'); const generateOtp = require('../utils/generateOtp'); const sendSms = require('../utils/sendSms'); const { issueTokens, accessToken } = require('../utils/generateTokens');
...
exports.forgotPassword = async (req, res) => { const { User } = getModels(); const phoneNumber = phoneOf(req.body.phoneNumber); const user = await User.findOne({ where: { phoneNumber, authProvider: 'local' } }); let code; if (user) ({ code } = await createOtp(phoneNumber, 'password_reset')); return success(res, 'If an eligible account exists, a password reset code has been sent.', {}, code); };
exports.verifyResetCode = async (req, res) => { const phoneNumber = phoneOf(req.body.phoneNumber); const checked = await verifyOtp(phoneNumber, req.body.code, 'password_reset'); if (checked.error) return res.status(400).json({ success: false, message: checked.error[1], code: checked.error[0], errors: [] }); const recoveryToken = jwt.sign({ phoneNumber, purpose: 'password_reset' }, process.env.JWT_SECRET, { expiresIn: '10m' }); return success(res, 'Reset code verified.', { recoveryToken }); };
exports.resetPassword = async (req, res) => { let payload; try { payload = jwt.verify(req.body.recoveryToken, process.env.JWT_SECRET); } catch (e) { return res.status(401).json({ success: false, message: 'Invalid or expired recovery token.', code: e.name === 'TokenExpiredError' ? 'TOKEN_EXPIRED' : 'TOKEN_INVALID', errors: [] }); } const phoneNumber = phoneOf(req.body.phoneNumber); if (payload.phoneNumber !== phoneNumber || payload.purpose !== 'password_reset') return res.status(401).json({ success: false, message: 'Recovery token does not match this request.', code: 'TOKEN_INVALID', errors: [] }); const { User, RefreshToken } = getModels(); const user = await User.findOne({ where: { phoneNumber, authProvider: 'local' } }); if (!user) return res.status(400).json({ success: false, message: 'Account not found.', code: 'INVALID_CREDENTIALS', errors: [] }); user.passwordHash = await bcrypt.hash(req.body.newPassword, 12); await user.save(); await RefreshToken.destroy({ where: { userId: user.id } }); return success(res, 'Password updated. Please log in again.', {}); };
