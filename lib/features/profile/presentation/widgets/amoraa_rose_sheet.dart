...
typedef RoseSender = Future<void> Function(String note);

Future<bool> showAmoraaRoseSheet({
  required BuildContext context,
  required String recipientName,
  required RoseSender onSend,
}) async {
  return await showAmoraBottomSheet<bool>(
        context: context,
        child: AmoraaRoseSheet(recipientName: recipientName, onSend: onSend),
      ) ??
      false;
}

class AmoraaRoseSheet extends StatefulWidget {
  const AmoraaRoseSheet({
    super.key,
  ...
  final RoseSender onSend;

  @override
  State<AmoraaRoseSheet> createState() => _AmoraaRoseSheetState();
}

class _AmoraaRoseSheetState extends State<AmoraaRoseSheet>
  ...
  Future<void> _send() async {
    ...
    try {
      await widget.onSend(_noteController.text.trim());
    } catch (error) {
      if (!mounted) return;
      setState(() {
        _sending = false;
        _error = userFacingErrorMessage(
          error,
          fallback: 'Couldn\'t send the Rose. Please try again.',
        );
      });
      return;
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Semantics(
                liveRegion: _sent,
                label: _sent ? 'Rose sent to $firstName' : 'Rose preview',
              ...
              TextField(
                ...
                minLines: 2,
                maxLines: 4,
                maxLength: AmoraaRoseSheet.maximumNoteLength,
